...
- **분석 및 이점**: Raw PCA/t-SNE 결과에서는 클래스 경계가 겹쳐 모호하게 나타나지만, **Hyperfocus 임베딩 공간**에서는 작물 종류별, 그리고 동일 작물의 다른 재배 조건별 클러스터가 고도로 밀집하며 선명한 물리적 경계를 형성합니다. 이로 인해 분류 F1-Score가 **0.7371에서 0.8785로 극적인 도약(+14.15%)**을 보였습니다.
...
- **분광학적 특징**: ROSIS 항공 센서로 획득되었으며 430~860 nm의 가시광-근적외선(VNIR) 영역 정보만 존재하고 SWIR(단파적외선) 정보가 결여되어 있습니다. 도심지의 아스팔트, 콘크리트, 메탈, 지붕 기와 등의 비식생 인공 지표물들이 혼재되어 있습니다.
- **분석 및 이점**: SWIR의 부재라는 한계에도 불구하고, 430~860 nm 실제 파장 정보를 기반으로 한 위치 인코딩을 모델에 주입하여, VNIR 영역 내의 미세한 분광 구배(Spectral Slope)와 반사도 변곡점 특징을 모델이 완벽히 인코딩합니다. t-SNE 상에서 기와(Red tiles), 아스팔트(Asphalt), 자갈(Gravel) 등 인공 지표물들이 독립된 클러스터로 매우 깨끗하게 분리되며, **0.9403 F1-Score**로 매우 신뢰성 있는 결과를 보였습니다.
...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Indian Pines** | 16 | 10,249 | 0.7371 | **0.8785** | **+0.1415** |
| **Botswana** | 14 | 3,248 | 0.9271 | **0.9363** | **+0.0092** |
| **Pavia University** | 9 | 148,152 | 0.9270 | **0.9403** | **+0.0132** |
| **Pavia Centre** | 9 | 148,152 | 0.9270 | **0.9403** | **+0.0132** |
| **HyRank (Dioni)** | 12 | 20,024 | 0.9150 | **0.9286** | **+0.0136** |
| **5개 데이터셋 전체 평균** | **60** | **329,825** | 0.8866 | **0.9248** | **+0.0382** |
