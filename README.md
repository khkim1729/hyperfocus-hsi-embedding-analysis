...
* 📄 **[교차 데이터셋 GNN 모델 제로샷 추론 분석 보고서 (gnn_cross_dataset_inference_analysis.md)](reports/gnn_cross_dataset_inference_analysis.md)**
...
14. [교차 데이터셋 GNN 모델 제로샷 추론 분석 (New Report)](#14-교차-데이터셋-gnn-모델-제로샷-추론-분석-new-report)
...
## 14. 교차 데이터셋 GNN 모델 제로샷 추론 분석 (New Report)

특정 초분광 데이터셋(Indian Pines, Pavia University, HyRank)의 원본 지표 클래스(Real Classes)로 학습된 GNN 모델을 외부의 전혀 다른 타겟 데이터셋에 직접 적용해 제로샷 추론을 실행하고, 상위 3개 예측 클래스 및 확률 분포를 분석하여 초분광 임베딩 공간의 도메인 일반화 거동을 규명하였습니다.

각 대상 데이터셋별 다중 분류 제로샷 전이 결과와, 타겟 클래스(예: Botswana)가 소스 도메인의 어떤 세부 지표 클래스로 투영되는지에 대한 시맨틱 정합성 분석 테이블은 아래 보고서에서 확인하실 수 있습니다.

👉 **[교차 데이터셋 GNN 모델 제로샷 추론 분석 보고서 바로가기 (reports/gnn_cross_dataset_inference_analysis.md)](reports/gnn_cross_dataset_inference_analysis.md)**
